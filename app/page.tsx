...
const suggestions = ["Explain a concept", "Analyze my code", "Design a UI", "Find bugs", "Improve performance"];
...
function SourceInput({ inputRef, onChange }: { inputRef: RefObject<HTMLInputElement | null>; onChange: (event: ChangeEvent<HTMLInputElement>) => void }) {
...
export default function Page() {
  ...
  useEffect(() => { if (window.localStorage.getItem("auren-theme") === "dark") setDark(true); try { setSavedProjects(JSON.parse(window.localStorage.getItem("auren-projects") || "[]")); } catch { setSavedProjects([]); } void loadStatus(); }, []);
  ...
  function toolPage() {
    ...
    if (["ui","bugs","performance","deployments"].includes(page)) {
      const config: Record<string,[string,string,string]> = { ui:["UI Generator","Describe the screen, app or component you want Auren to design.","ui"], bugs:["Bug Finder","Paste an error, behavior, stack trace or code context. Auren will rank likely causes and fixes.","bugs"], performance:["Performance","Describe the slow page, endpoint, component or workload. Include metrics or code when available.","performance"], deployments:["Deployments","Describe your deployment target and configuration. Auren will produce a release-readiness plan; it will not claim to deploy anything.","deployments"] };
      const [title,desc,task] = config[page]; return <div className="dashboard"><div className="serif">{title}</div><p className="muted">{desc}</p><div className="card"><textarea className="task-area" rows={7} value={taskPrompt} onChange={(e)=>setTaskPrompt(e.target.value)} placeholder="Describe what you need…"/><div className="action-row"><button className="primary" onClick={()=>void runTask(task)}>Run with Auren</button><button className="secondary" onClick={()=>void runTask(task, task === "ui" ? "Design a clean dashboard for a context-aware AI assistant." : "Analyze a small example and show the recommended approach.")}>Use example</button></div></div><ResultCard title="Auren result" text={taskResult} loading={taskLoading}/></div>;
    }

    if (page === "projects") return <div className="dashboard"><div className="dash-head"><div><div className="serif">My Projects</div><p className="muted">Projects saved in this browser.</p></div><button className="primary" onClick={()=>setPage("new")}>New project</button></div>{savedProjects.length ? <div className="projects">{savedProjects.map((project,index)=><button className="card project" key={`${project.name}-${index}`} onClick={()=>{setTaskPrompt(`Review the project named ${project.name} and suggest the next engineering steps.`);setPage("assistant");}}><div className="project-banner"/><div className="project-body"><div className="project-title">{project.name}</div><div className="muted">{project.files} files · {project.createdAt}</div><span className="pill">Open in Auren</span></div></button>)}</div> : <div className="empty">No saved projects yet. Create a project and save it after analysis.</div>}</div>;

    if (page === "integrations") return <div className="dashboard"><div className="serif">Integrations</div><p className="muted">Provider keys are checked on the server. Secret values are never displayed.</p><div className="integration-grid">{models.filter((item)=>item.id !== "auto").map((item)=>{const key=item.id;const connected=status?.providers[key];return <div className="card integration" key={item.id}><div><h3>{item.name}</h3><p className="muted">{item.desc}</p></div><span className={connected ? "status connected" : "status"}>{connected ? "Connected" : "Not configured"}</span><small>{status?.models[key] || "Checking…"}</small></div>})}</div><div className="card"><h3>Auren Auto</h3><p className="muted">Auren Auto asks every configured provider in parallel, compares the answers, and uses Gemini 3.8 Flash as the preferred synthesis model when it is configured.</p><button className="secondary" onClick={()=>void loadStatus()}>Refresh status</button></div></div>;

    if (page === "settings") return <div className="dashboard"><div className="serif">Settings</div><p className="muted">Local interface preferences and model selection.</p><div className="card setting-row"><div><h3>Theme</h3><p className="muted">Saved in this browser.</p></div><button className="secondary" onClick={()=>setDark(!dark)}>{dark ? <Sun size={15}/> : <Moon size={15}/>} {dark ? "Light mode" : "Dark mode"}</button></div><div className="card"><h3>Default model</h3><p className="muted">Recommended: Auren Auto. It preserves the core Auren multi-model comparison workflow.</p><div className="model-list">{models.map((item)=><button key={item.id} className={model===item.id ? "model-choice selected" : "model-choice"} onClick={()=>setModel(item.id)}><b>{item.name}</b><span>{item.desc}</span></button>)}</div></div><div className="card"><h3>Provider keys</h3><p className="muted">Keys are configured in <code>.env.local</code>. Never paste API keys into chat.</p><button className="secondary" onClick={()=>setPage("integrations")}>Open integrations</button></div></div>;

    return <div className="dashboard"><div className="serif">Auren</div><div className="empty">Choose a workspace section from the sidebar.</div></div>;
  }

  const selectedModel = models.find((item)=>item.id===model) || models[0];
  const connectedCount = status ? Object.entries(status.providers).filter(([key,value])=>key !== "demo" && value).length : 0;

  return <div className="app" style={vars as CSSProperties}>
    {side && <aside className="sidebar"><div className="brand"><div className="brand-row"><span className="brand-dot"><i/></span><span className="brand-name">Auren</span></div><div className="tagline">Context-aware conversational visual intelligence</div>{page === "home" && <button className="new-chat" onClick={()=>{setMessages([]);setFiles([]);setInput("");}}>＋ New conversation</button>}</div><nav className="nav">{nav.map(([Icon,label,key])=><button key={key} title={`Open ${label}`} className={page===key ? "active" : ""} onClick={()=>setPage(key)}><Icon size={15}/>{label}</button>)}</nav><div className="history"><div className="history-label">Recent</div>{["Explain this code","Design a login page","Find bugs in my API","Plan a deployment"].map((item)=><button key={item} onClick={()=>{setPage("home");setInput(item);}}><FileText size={13}/>{item}</button>)}</div><div className="upgrade"><strong>{connectedCount > 0 ? `${connectedCount} live provider${connectedCount === 1 ? "" : "s"}` : "Connect an AI provider"}</strong><p>{connectedCount > 0 ? "Auren is ready for live model requests." : "Add a provider key to .env.local and restart the server."}</p></div></aside>}
    <main className="main"><header className="topbar"><div className="top-left"><button className="icon" onClick={()=>setSide(!side)} title="Toggle sidebar">{side ? <PanelLeftClose size={17}/> : <PanelLeftOpen size={17}/>}</button><div style={{position:"relative"}}><button className="select" onClick={()=>setModelOpen(!modelOpen)}>{selectedModel.name} <ChevronDown size={13}/></button>{modelOpen && <div className="card model-menu">{models.map((item)=><button key={item.id} className={item.id===model ? "model-choice selected" : "model-choice"} onClick={()=>{setModel(item.id);setModelOpen(false);}}><b>{item.name}</b><span>{item.desc}</span></button>)}</div>}</div></div><div className="top-left"><span className="status connected">{connectedCount > 0 ? "Live" : "Demo"}</span><button className="icon" onClick={()=>setDark(!dark)} title="Toggle theme">{dark ? <Sun size={17}/> : <Moon size={17}/>}</button></div></header>
      <section className="content">{page === "home" ? <div className="home"><div className="chat"><div className="chat-inner">{!messages.length && <><div className="hello">Hello. I’m Auren.</div><div className="muted" style={{marginBottom:18}}>Ask one question. Auren can query your configured AI models, compare their answers, and return one final response.</div><div className="card" style={{padding:14,marginBottom:18}}><b>{connectedCount > 0 ? "Live AI ready" : "Demo mode"}</b><div className="muted" style={{marginTop:5}}>{connectedCount > 0 ? `${connectedCount} provider${connectedCount === 1 ? " is" : "s are"} configured. Recommended mode: Auren Auto.` : "No provider key is available to the server yet."}</div></div><div className="chips">{suggestions.map((suggestion)=><button className="chip" key={suggestion} onClick={()=>setInput(suggestion)}>{suggestion}</button>)}</div></>}{messages.map((message,index)=><div className={`message ${message.role}`} key={`${message.role}-${index}`}><div className="role">{message.role === "user" ? "You" : "Auren"}{message.meta ? ` · ${message.meta}` : ""}</div><FilePills files={message.files || []}/><div>{message.text}</div></div>)}{thinking && <div className="message"><div className="role">Auren</div><div>Asking configured model(s), comparing answers, and synthesizing…</div></div>}</div></div><div className="composer-wrap"><div className="composer"><FilePills files={files} removable onRemove={(file)=>setFiles((current)=>current.filter((item)=>item!==file))}/><textarea rows={2} value={input} onChange={(event)=>setInput(event.target.value)} onKeyDown={(event)=>{if(event.key === "Enter" && !event.shiftKey){event.preventDefault();void send();}}} placeholder="Message Auren…"/><div className="composer-bottom"><div><input ref={fileRef} hidden type="file" multiple accept={ACCEPTED_SOURCE} onChange={async(event)=>setFiles(await readFiles(event.target.files))}/><button className="icon" onClick={()=>fileRef.current?.click()} title="Attach source files"><Paperclip size={16}/></button></div><button className="send" onClick={()=>void send()} disabled={thinking} title="Send"><ArrowUp size={15}/></button></div></div></div></div> : toolPage()}</section></main>
  </div>;
}
